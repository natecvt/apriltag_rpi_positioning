...
int main(int argc, char* argv[])
{
    ...
    if (s.calibrationPattern == SettingsCV::Pattern::CHARUCOBOARD) {
    ...
    if (s.calibrationPattern == SettingsCV::CHARUCOBOARD) {
        if (s.arucoDictFileName == "") {
            cv::aruco::PREDEFINED_DICTIONARY_NAME arucoDict;
    ...
    cv::aruco::CharucoBoard ch_board;
    ch_board.create(s.boardSize.width, s.boardSize.height, s.squareSize, s.markerSize, &dictionary);
    cv::aruco::DetectorParameters ch_detector;
    ch_detector.create();
    ...
    for(;;)
    {
        GstSample *sample = gst_app_sink_try_pull_sample(GST_APP_SINK(streams.sink), GST_SECOND);
        ...
        case SettingsCV::CHESSBOARD:
            ...
            break;
        case SettingsCV::CHARUCOBOARD:
            cv::aruco::detectMarkers( view, ch_board.dictionary, pointBuf, markerIds, &ch_detector );
            found = pointBuf.size() == (size_t)((s.boardSize.height - 1)*(s.boardSize.width - 1));
            break;
        case SettingsCV::CIRCLES_GRID:
            found = findCirclesGrid( view, s.boardSize, pointBuf );
            break;
        case SettingsCV::ASYMMETRIC_CIRCLES_GRID:
    ...
    //! [show_results]
    if( s.inputType == SettingsCV::IMAGE_LIST && s.showUndistorted && !cameraMatrix.empty())
    {
...
static void calcBoardCornerPositions(Size boardSize, float squareSize, vector<Point3f>& corners,
                                     SettingsCV::Pattern patternType /*= SettingsCV::CHESSBOARD*/)
{
    corners.clear();
...
static void saveCameraParams( SettingsCV& s, Size& imageSize, Mat& cameraMatrix, Mat& distCoeffs,
                              const vector<Mat>& rvecs, const vector<Mat>& tvecs,
